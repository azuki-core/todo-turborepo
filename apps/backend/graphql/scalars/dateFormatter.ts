...
export const parseTime = (time: string): Date => {
  ...
  return new Date(currentDateString.slice(0, Math.max(0, currentDateString.indexOf('T') + 1)) + time)
}
...
export const serializeTimeString = (time: string): string => {
  ...
  const regexFracSec = /\.\d+/

  // Retrieve the fractional second part of the time
  // string if it exists.
  const fractionalPart = regexFracSec.exec(time)
...
export const serializeDate = (date: Date): string => date.toISOString().split('T')[0]!

// Parses an RFC 3339 compliant date-time-string into a Date.
export const parseDateTime = (dateTime: string): Date => new Date(dateTime)

// Serializes a Date into an RFC 3339 compliant date-time-string
// in the format YYYY-MM-DDThh:mm:ss.sssZ.
export const serializeDateTime = (dateTime: Date): string => dateTime.toISOString()
...
export const serializeDateTimeString = (dateTime: string): string => {
  ...
  // Retrieve the fractional second part of the time
  // string if it exists.
  const fractionalPart = regexFracSec.exec(dateTime)
  if (fractionalPart === null) {
...
// in the format YYYY-MM-DDThh:mm:ss.sssZ
export const serializeUnixTimestamp = (timestamp: number): string => new Date(timestamp * 1000).toISOString()
